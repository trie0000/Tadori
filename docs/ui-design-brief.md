...
- 結果リスト: 1 行 = 件名（強調）/ 送信者・日時（muted）/ 本文スニペット（--hl で
  クエリ周辺をハイライト）/ 右に「OWA で開く」ボタン。
- 各行クリックで詳細プレビュー（右ペイン or 展開）。
- **★OWA で開く の挙動（Spira の教訓・重要）**: OWA は検索状態のディープリンク
  （`outlook.office.com/mail/?q=...` 等）を**信頼できる形で提供していない**（cloud.microsoft
  移行後のテナントで動かない）。Spira が採用した確実なパターンを踏襲する:
  **「`from:<送信者> received:<受信日>` をクリップボードにコピー + OWA 受信箱を新タブで開く」**
  → ユーザが ⌘/Ctrl+V → Enter。UI 上は「OWA で開く」ボタン押下でトースト「検索条件を
  コピーしました。OWA で貼り付けて検索してください」を出す設計にする。直リンクに見せない。
...
> ブランド絵文字を使わない。検索結果の「OWA で開く」は直リンクでなく
> 「検索条件コピー＋受信箱を開く」パターン。
