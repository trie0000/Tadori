...
- **入力は自然言語の質問**（キーワード/AND-OR 検索ではない）。「VPN 在宅 切れる」のような
  単語列でなく、「在宅でVPNが切れる時の対処は？」のような文で受ける。内部はクエリを埋め込み
  →ベクトル類似で意味検索。プレースホルダ・ヒントも「自然文で質問」と明示する。
- スニペットの `--hl` ハイライトは**意味的に関連する箇所**を示すもので、入力語の字面一致では
  ない（字面が出てこなくてもヒットしうる）。
