...
### (A) 検索パネル（チャット形式）★最重要
**検索はチャット UI**。1 回の検索 = 1 ターン（質問バブル + 結果群）。過去のやり取りは
スレッドとして残り、上にスクロールすると遡れる。LLM 回答合成はしない（現状は検索結果の
提示のみ。Phase5 で Q&A 拡張余地）。

- **スレッド領域**（スクロール）: ターンが縦に積まれる。
  - 質問バブル: 右寄せ・accent-soft 背景・角丸（吹き出し）。ユーザの検索クエリ。
  - 回答メタ行: 「1,240 件中 上位 N 件 · 38ms」（mono）。
  - 結果カード: 件名（強調）/ 送信者・日時（muted）/ 本文スニペット（--hl でクエリ周辺
    ハイライト）。クリックで本文展開（List の同じ行から取得）。
...
- ※ OWA 連携はスコープ外。検索もプレビューも Tadori パネル内で完結。
